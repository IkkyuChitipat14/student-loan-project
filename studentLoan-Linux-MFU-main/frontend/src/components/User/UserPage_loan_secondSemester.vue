<template>
  <div>
    ...
    <UserPopUpSecondTermLoan ref="loanModal" :studentID="studentID" />
  ...
  <div class="screen-control">
    ...
    <div class="p-3 text-black">
      <div class="bg-white shadow">
        <div class="p-2 row">
          <div class="col">
            <h5 class="my-auto p-2">ยื่นกู้</h5>
          </div>
          <div class="col-md my-auto mx-2">
            <div class="row">
              <div class="col-auto">
                <p class="m-0">สถานะการกู้:</p>
              </div>
              <div class="col fw-bolder" id="info_loan_status"></div>
            </div>
          </div>
        </div>
        <hr
    ...
    <!-- Approve Box -->
    <div class="px-3 text-black" v-if="loan_status_apporve">
      <button
        class="btn btn-info d-flex align-items-center float-end px-4 py-2"
        :disabled="loan_status_apporve == false || sentFormSuccess"
...
import UserPopUpSecondTermLoan from "./User_popUp_secondTerm_Loan.vue";
...
export default {
  ...
  methods: {
    ...
    async getRejectCheck() {
      try {
        // Properly append the query string with a key
        const response = await fetch(
          `${process.env.VUE_APP_API_BASE_URL}/user/reject/toLoan/check`,
          {
            method: "GET",
            credentials: "include",
          }
        ...
        if (response.ok) {
          ...
          if (staticData.message == "not found") {
            return this.$refs.loanModal.openRejectBox();
          }
          if (staticData.loaning == false) {
            await Swal.fire({
              title: "คุณได้ปฏิเสธการกู้ยืมเงินแล้ว",
              text: "กรุณาติดต่อเจ้าหน้าที่ กยศ. หากคุณมีความประสงค์ที่จะยื่นกู้ยืมอีกครั้ง",
              icon: "warning",
    ...
    async getLoanData(value) {
      try {
        // Properly append the query string with a key
        const response = await fetch(
          `${process.env.VUE_APP_API_BASE_URL}/user/get/loan/data?id=${encodeURIComponent(value)}`,
          {
            method: "GET",
            credentials: "include",
          }
    ...
    async getStudentLoanLast() {
      try {
        ...
        if (response.ok) {
          ...
          this.public_id = staticData.public_id;
          this.getLoanData(staticData.public_id);
          if (staticData.loan_status == "อยู่ระหว่างดำเนินการ") {
            await Swal.fire({
            ...
            return this.$router.replace("/user");
          } else if (staticData.loan_status == "อนุมัติ") {
            if (staticData.user_semesterStudy == "2") {
              await Swal.fire({
                title: "คำขอกู้ยืมของคุณได้รับการอนุมัติแล้ว",
                text: "",
                icon: "success",
                confirmButtonText: "ยืนยัน",
                confirmButtonColor: "#964040",
            ...
            return this.getRejectCheck();
          } else {
            await Swal.fire({
              title: "คำขอกู้ยืมของคุณไม่ได้รับการอนุมัติ",
              confirmButtonText: "ยืนยัน",
              confirmButtonColor: "#964040",
            });
            this.$nextTick(() => {
    ...
    Info_show_data(datas) {
      ...
      // show loan status
      switch (formData.loan_status) {
        case "อยู่ระหว่างดำเนินการ":
          setStatus.innerHTML = `<p style="color: orange;" class="my-auto m-0">${formData.loan_status}</p>`;
          break;
        case "อนุมัติ":
          setStatus.innerHTML = `<p class="text-success my-auto m-0">${formData.loan_status}</p>`;
          this.loan_status_apporve = true;
          break;
        case "ตีกลับเพื่อแก้เอกสาร":
          setStatus.innerHTML = `<p class="text-primary my-auto m-0">${formData.loan_status}</p>`;
          break;
        case "ไม่อนุมัติ":
          setStatus.innerHTML = `<p class="text-danger my-auto m-0">${formData.loan_status}</p>`;
          break;
        default:
          break;
      }
      ...
      const setText = (id, text) => {
        ...
        if (element) element.innerText = text ? text : "-";
      };
    ...
    async postSubmitForm() {
      const canSubmit = await this.getRejectCheck();
      if (!canSubmit) return;
      try {
        // Properly append the query string with a key
        const response = await fetch(
          `${process.env.VUE_APP_API_BASE_URL}/user/loan/submit_form/secondSemester`,
          {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ public_id: this.public_id }),
          }
        );
        if (response.ok) {
          const res = await response.json();
          this.sentFormSuccess = true;
  ...
  mounted() {
    ...
    // Retrieve `uld.id` from localStorage
  },
  ...
  unmounted() {
    ...
    // Clear specific timeouts if they exist
    if (this.timeoutId) {
      clearTimeout(this.timseoutId);
      this.timeoutId = null;
    }
